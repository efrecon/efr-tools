...
proc ::init::init { args } {
	...
	switch -- $opt {
		...
		::bootstrap::log debug "Using $ARGS(-store) as default global\
                                        storage array"
	    }
	    -log {
	...
	if { $::tcl_platform(platform) eq "windows" } {
		...
		::bootstrap::log warn "Cannot open console, running under wish?\
                                       $err"
	    }
	}
    }
	...
	if { [catch {eval $ARGS(-parsed) $ARGS(-store) $argstore} err] } {
	    ::bootstrap::log error "Could not execute parsed callback: $err"
	}
    }
    ...
    $GLBL(log)::info "Initial language for application will be: $lang"
    if { $ARGS(-splash) ne "" } {
	...
	if { [catch {eval $ARGS(-argsread) $ARGS(-store) $argstore} err] } {
	    ::bootstrap::log error "Could not execute args read callback: $err"
	}
    }
	...
	if { [catch {::tkconclient::start $GLBL(dbgport)} err] } {
	    $GLBL(log)::warn "Could not start remote debug facility: $err"
	}
    }
    ...
    # Install local language settings.
    set lang [__localisation $GLBL(language)]
    $GLBL(log)::info "Language for application will be: $lang"
    if { $ARGS(-splash) ne "" } {
	::splash::progress $splash "Language for application $lang"
    }
	...
	if { $ARGS(-callback) ne "" } {
		...
		$GLBL(log)::error "Could not execute advance callback: $err"
	    }
	}
	if { $ARGS(-splash) ne "" } {
